...
// 寄給業主郵件
function industry_generateMailOptions(recipientData, ordering_person_data, carData) {
    const recipientInfo = generate_recipient_info(recipientData)
...
function send_shopping_mail(carData, recipientData, ordering_person_data) {
    return new Promise((resolve, reject) => {
        const tableContent = generate_table(carData)
        const totalAmount = calculate_total_amount(carData)
        const mail_options = generate_mail_options(recipientData, tableContent, totalAmount)
        const industry_mailOptions = industry_generateMailOptions(recipientData, ordering_person_data, carData)
