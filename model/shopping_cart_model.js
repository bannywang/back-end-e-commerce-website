...
async function create_shopping_cart(user_id, product_id, size_id, quantity) {
    try {
        ...
        const [price_results] = await connection.query(get_price_query, get_price_values)
        ...
        const [car_results] = await connection.query(check_car_query, check_car_values)
        ...
        if (car_results.length > 0) {
            ...
            const update_query = `
                ...
                WHERE user_id = ? AND product_id = ? AND size_id = ? AND shopping_status = 1
            `
            const update_values = [new_quantity, new_subtotal, new_final_price, user_id, product_id, size_id]
        ...
        const insert_query = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `
        const insert_values = [user_id, product_id, size_id, quantity, price, subtotal, final_price]
...
async function change_the_number_of_shopping_carts(user_id, shopping_cart_id, new_quantity) {
    try {
        ...
        const [select_results] = await connection.query(select_query, select_values)

        if (select_results.length === 0) {
            console.error('找不到符合指定的購物車項目，或該項目的 shopping_status 不為 1')
            return false
        }
        ...
        const update_values = [new_quantity, subtotal, final_price, shopping_status, shopping_cart_id, user_id]
        await connection.query(update_query, update_values)
...
async function product_total_in_shopping_cart(user_id) {
    try {
        ...
        const query_cart_totals = `
            ...
            WHERE user_id = ? AND shopping_status = 1
        `
        const [results_cart_totals] = await connection.query(query_cart_totals, [user_id])
        const final_total = parseFloat(results_cart_totals[0].final_total) || 0
        const original_total = parseFloat(results_cart_totals[0].original_total) || 0
...
async function delete_shopping_data_by_user_id(user_id, shopping_cart_id) {
    try {
        ...
        console.log(`已將購物車第 ${shopping_cart_id} 筆購物車資料的 shopping_status 改為 0`)
...
// 結帳功能
async function checkout_shopping_cart(user_id, recipient_name, recipient_phone, recipient_address, shopping_cart_ids) {
    const conn = await connection.getConnection()
    try {
        // 開始事務
        await conn.beginTransaction()
        const ids_array = Array.isArray(shopping_cart_ids) ? shopping_cart_ids : [shopping_cart_ids]
        ...
        for (const shopping_cart_id of ids_array) {
            ...
            `
            const [shopping_cart_results] = await connection.query(get_shopping_cart_query, [user_id, shopping_cart_id])

            if (shopping_cart_results.length === 0) {
                console.error(`找不到購物車 ID 為 ${shopping_cart_id} 的資料或購物車狀態不正確`)
                continue // 跳過處理此購物車，繼續處理下一筆
            }

            const shopping_cart = shopping_cart_results[0]
            const { quantity, price, subtotal, final_price, product_id, size_id } = shopping_cart

            // 檢查商品庫存是否足夠
            const available_stock = await product.get_single_product_single_size_stock_quantity(product_id, size_id)
            if (available_stock.length === 0 || available_stock[0].stock_quantity < quantity) {
                console.error(`商品庫存不足 (商品ID: ${product_id} 尺寸ID: ${size_id})`)
                return false // 結帳失敗
            }
            ...
            const [insert_result] = await connection.query(insert_purchase_history_query, [
                user_id,
                recipient_name,
                recipient_phone,
                recipient_address,
            ...
            console.log(`結帳成功，已成功寫入歷史訂單 (購物車 ID: ${shopping_cart_id})`)
