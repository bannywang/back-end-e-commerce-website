...
async function register_user(account, password, name, phone, mail) {
    try {
        ...
        const account_check_query = `
          ...
          WHERE account = ?
      `
        const [account_check_results] = await connection.query(account_check_query, [account])
        ...
        const phone_check_query = `
          ...
          WHERE phone = ?
      `
        const [phone_check_results] = await connection.query(phone_check_query, [phone])
        ...
        const mail_check_query = `
          ...
          WHERE mail = ?
      `
        const [mail_check_results] = await connection.query(mail_check_query, [mail])
...
async function login(user_account, password) {
    try {
        const [results] = await connection.query('SELECT * FROM `user_data` WHERE `account` = ? AND `password` = ?;', [
            user_account,
            password,
        ])
        ...
        // 修改 login_status 為 1
        const update_result = await connection.query('UPDATE `user_data` SET `login_status` = 1 WHERE `id` = ?;', [
            user.id,
        ])
...
async function logout(user_id) {
    try {
        // 檢查使用者是否存在
        const [results] = await connection.query('SELECT * FROM `user_data` WHERE `id` = ?;', [user_id])
        ...
        // 修改 login_status 為 0
        const update_result = await connection.query('UPDATE `user_data` SET `login_status` = 0 WHERE `id` = ?;', [
            user_id,
        ])
...
async function use_account_logout(user_account) {
    try {
        // 修改 login_status 為 0
        const update_result = await connection.query('UPDATE `user_data` SET `login_status` = 0 WHERE `account` = ?;', [
            user_account,
        ])
