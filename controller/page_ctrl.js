...
async function product_page(req, res) {
    try {
        ...
        const only = await product_data.get_single_product_single_size_stock_quantity(id, 7)
        console.log(only)
...
async function history_page(req, res) {
    ...
    try {
        if (id_card) {
            const history = await purchase_history.get_detailed_purchase_history(id_card.id)
...
async function car_page(req, res) {
    ...
    try {
        if (id_card) {
            ...
            const shopping_cart = await shopping_car.get_user_data_from_shopping_cart(id_card.id)
            const shopping_cart_sum = await shopping_car.product_total_in_shopping_cart(id_card.id)
