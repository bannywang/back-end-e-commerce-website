...
async function add_to_car(req, res) {
    ...
    try {
        ...
        // 在資料庫中創建購物車項目
        const result = await shopping.create_shopping_cart(user, product, size, quantity)
...
async function update_quantity(req, res) {
    ...
    try {
        ...
        // 在資料庫中異動購物車項目
        const new_quantity = await shopping.change_the_number_of_shopping_carts(user, product, quantity)
        ...
        if (!new_quantity) {
            ...
            const shopping_cart = await shopping.get_user_data_from_shopping_cart(user)
            //折扣後金額
            const shopping_cart_sum = await shopping.product_total_in_shopping_cart(user)
...
async function delete_car_product(req, res) {
    ...
    try {
        ...
        const delete_result = await shopping.delete_shopping_data_by_user_id(user_id, car_id)
        console.log('刪除結果：', delete_result)
        const user = await user_data.use_id_get_user_data(user_id)
        const grade = user[0].grade
        const shopping_cart = await shopping.get_user_data_from_shopping_cart(id_card.id)
        const shopping_cart_sum = await shopping.product_total_in_shopping_cart(user_id)
...
async function buy_car_product(req, res) {
    ...
    try {
        ...
        // 在資料庫中創建購買紀錄
        const record_result = await shopping.checkout_shopping_cart(user_id, name, phone, address, shopping_car_id)
